...
import asyncio, re, ast, os, pyrogram
...
DELETE_TIME = os.environ.get("DELETE_TIME")
...
async def advantage_spoll_choker(bot, query):
    ...
    if k==False:
        files, offset, total_results = await get_search_results(movie, offset=10, filter=True)
        if files:
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        cap = f"""**📁 Here is What I Found In My Database For Your Query : #{query} 👇**
...
async def advantage_spell_chok(msg):
    ...
    await asyncio.sleep(10)
    await advantage_spell_chok.delete()
